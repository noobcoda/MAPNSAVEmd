...
class Calculation:
    def __init__(self):
        self.toBeSortedListPrices = []

    def getFinalList(self):
        return self.toBeSortedListPrices

    def add_to_be_sorted(self,aDict,store_obj):
        for product,price in aDict.items():
            self.toBeSortedListPrices.append((product,float(eval(price)),store_obj))  # so it looks like (('tesco biscuit',1.00),((tesco biscuit 2',1.50))
    ...
    def mergesort(self,randomList):
        ...
        if len(randomList) <=1:
            ...
            #does left side of array
            left = self.mergesort(randomList[:mid])

            #does right side of array
            right = self.mergesort(randomList[mid:])
